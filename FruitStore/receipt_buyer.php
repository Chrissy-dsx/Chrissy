...
<body>
        ...
        if(!empty($cart_items)){
            foreach ($cart_items as $fruit) {
                ...
                if ($fruit_amount > 0) {
                    ...
                    $database_amount=$row["Inventory"];
                    $assume_ammount=$database_amount-$fruit_amount;
                    if($database_amount<=0){
                        echo "<script>alert('Sorry,the item is out of stock'); location.href=\"store_cart.php\"</script>";
                    }
                    else if($assume_ammount<0){
                        echo "<script>alert('Sorry,the inventory is not enough for the supply'); location.href=\"store_cart.php\"</script>";
                    }
                        ...
                        //delete cart
                        $sql="DELETE FROM cart WHERE username='$usingname'";
                        mysqli_query($conn, $sql);
                    }
                }
    ...
    <a href="store_buyer.php"><button>back</button></a>
</body>
</html>